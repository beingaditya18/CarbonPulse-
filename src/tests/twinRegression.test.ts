...
describe('Twin Regression Engine - Comprehensive Test Suite (23 Tests)', () => {
  ...
  // Test 16: fitRegression handles negative coordinates
  it('16. fitRegression handles negative coordinates (negative x values) correctly', () => {
    const data = [
      makeEmissionDataPoint({ x: -2, y: 1 }),
      makeEmissionDataPoint({ x: -1, y: 3 }),
      makeEmissionDataPoint({ x: 0, y: 5 }),
    ];
    const model = fitRegression(data);
    expect(model.slope).toBeCloseTo(2, 4);
    expect(model.intercept).toBeCloseTo(5, 4);
  });

  // Test 17: projectEmissions handles zero forecast days
  it('17. projectEmissions returns empty arrays when days is 0', () => {
    const model = fitRegression([
      makeEmissionDataPoint({ x: 0, y: 10 }),
      makeEmissionDataPoint({ x: 1, y: 20 }),
    ]);
    const projection = projectEmissions(model, 0);
    expect(projection.baseline).toHaveLength(0);
    expect(projection.simulated).toHaveLength(0);
  });

  // Test 18: projectEmissions handles 100% reduction target (1.0)
  it('18. projectEmissions sets simulated values to 0 when reduction factor is 1.0', () => {
    const model = fitRegression([
      makeEmissionDataPoint({ x: 0, y: 10 }),
      makeEmissionDataPoint({ x: 1, y: 20 }),
    ]);
    const projection = projectEmissions(model, 5, { transportation: 1.0 });
    projection.simulated.forEach(val => {
      expect(val).toBe(0);
    });
  });

  // Test 19: projectEmissions handles negative reduction targets (which increases emissions)
  it('19. projectEmissions increases simulated emissions when reduction target is negative', () => {
    const model = fitRegression([
      makeEmissionDataPoint({ x: 0, y: 10 }),
      makeEmissionDataPoint({ x: 1, y: 20 }),
    ]);
    const projection = projectEmissions(model, 5, { transportation: -0.2 });
    for (let i = 0; i < projection.baseline.length; i++) {
      expect(projection.simulated[i]).toBeGreaterThan(projection.baseline[i]);
    }
  });

  // Test 20: simulateTwinEmissions handles reduction percentage of 100%
  it('20. simulateTwinEmissions handles reduction percentage of 100% correctly', () => {
    const logs = [
      makeCarbonLog({ category: 'transportation', emission_amount: 100, logged_date: '2026-06-01T00:00:00.000Z' }),
      makeCarbonLog({ category: 'transportation', emission_amount: 110, logged_date: '2026-06-02T00:00:00.000Z' }),
      makeCarbonLog({ category: 'transportation', emission_amount: 120, logged_date: '2026-06-03T00:00:00.000Z' }),
    ];
    const result = simulateTwinEmissions(logs, 'transportation', 100, 30);
    expect(result.simulatedProjected).toBe(0);
    expect(result.carbonSaved).toBe(result.baselineProjected);
  });

  // Test 21: simulateTwinEmissions handles reduction percentage of 0%
  it('21. simulateTwinEmissions handles reduction percentage of 0% correctly', () => {
    const logs = [
      makeCarbonLog({ category: 'transportation', emission_amount: 100, logged_date: '2026-06-01T00:00:00.000Z' }),
      makeCarbonLog({ category: 'transportation', emission_amount: 110, logged_date: '2026-06-02T00:00:00.000Z' }),
      makeCarbonLog({ category: 'transportation', emission_amount: 120, logged_date: '2026-06-03T00:00:00.000Z' }),
    ];
    const result = simulateTwinEmissions(logs, 'transportation', 0, 30);
    expect(result.simulatedProjected).toBe(result.baselineProjected);
    expect(result.carbonSaved).toBe(0);
  });

  // Test 22: simulateTwinEmissions fallback model handles zero emission logs with 100% reduction
  it('22. simulateTwinEmissions fallback model handles zero emission logs with 100% reduction', () => {
    const logs = [
  ...
  // Test 23: fitRegression handles identical x coordinate data points
  it('23. fitRegression handles identical x coordinate data points by setting slope to 0', () => {
    const data = [
      makeEmissionDataPoint({ x: 5, y: 10 }),
      makeEmissionDataPoint({ x: 5, y: 20 }),
    ];
    const model = fitRegression(data);
    expect(model.slope).toBe(0);
    expect(model.intercept).toBe(15);
  });
});
