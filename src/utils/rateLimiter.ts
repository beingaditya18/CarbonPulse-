...
export function checkRateLimit(ip: string): boolean {
  ...
  // Bounded memory protection: prevent map from expanding indefinitely under high loads
  if (clientBuckets.size > 10000) {
    clientBuckets.clear();
  }

  // Periodic inline pruning: clean up fully refilled, idle buckets (approx. 2% chance per check)
  if (Math.random() < 0.02) {
    for (const [key, b] of clientBuckets.entries()) {
      if (b.tokens >= BUCKET_CAPACITY && now - b.lastRefill > REFILL_RATE_MS) {
