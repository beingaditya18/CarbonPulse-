 ...
 * Sum(SHAP values) = Predicted Footprint - Expected Baseline.
 * 
 * @param logs - User's carbon footprint activity logs
 * @param referenceDateOrOptions - Optional reference date or options containing custom region and baseline
 * @returns Object containing predicted emissions, base value, and explanations list
 */
export function calculateSHAPExplanations(
  logs: CarbonLog[],
  referenceDateOrOptions?: Date | {
    referenceDate?: Date;
  ...
  let customBaseline: number | undefined;
  ...
  if (referenceDateOrOptions instanceof Date) {
    ...
    customBaseline = referenceDateOrOptions.customBaseline;
  }
  ...
  // Scaling baseline contributions proportionally if a custom baseline is active
  const baseValue = customBaseline || EXPECTED_BASE_VALUE;
  const scaleFactor = baseValue / EXPECTED_BASE_VALUE;

  const scaled_SHAP_BASE_T = SHAP_BASE_T * scaleFactor;
  const scaled_SHAP_BASE_E = SHAP_BASE_E * scaleFactor;
  const scaled_SHAP_BASE_F = SHAP_BASE_F * scaleFactor;
  const scaled_SHAP_BASE_S = SHAP_BASE_S * scaleFactor;
  const scaled_SHAP_BASE_W = SHAP_BASE_W * scaleFactor;

  // Compute SHAP values relative to scaled regional/custom baselines
  const T_shap = T * COEFF_T + 0.5 * COEFF_INTERACTION * T * S - scaled_SHAP_BASE_T;
  const E_shap = E * COEFF_E - scaled_SHAP_BASE_E;
  const F_shap = F * COEFF_F - scaled_SHAP_BASE_F;
  const S_shap = S * COEFF_S + 0.5 * COEFF_INTERACTION * T * S - scaled_SHAP_BASE_S;
  const W_shap = W * COEFF_W - scaled_SHAP_BASE_W;
